...
open class DocumentPicker: NSObject {
    ...
    public func fileAction(for type: SourceType, title: String) -> UIAlertAction? {
        return UIAlertAction(title: title, style: .default) { [unowned self] _ in
            self.pickerController = UIDocumentPickerViewController(documentTypes: [kUTTypeMovie as String, kUTTypeImage as String, kUTTypeItem as String], in: .open)
            self.pickerController!.delegate = self
...
extension DocumentPicker: UIDocumentPickerDelegate{
    ...
    public func documentPicker(_ controller: UIDocumentPickerViewController, didPickDocumentsAt urls: [URL]) {
        ...
        /*let bucket = Storage.storage().reference()
               ...
               /* bucket.child("NEW").putFile(from: url, metadata: nil){
                    (_, err) in
                    if err != nil{
                ...
                */
        let videoRef = bucket.child("Hizmet1").child(url.deletingPathExtension().lastPathComponent)
         ...
         */
        delegate?.keepDocURL(documentURL: url)
